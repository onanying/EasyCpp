...
void easycpp::helpers::log_error(std::string tag, std::string msg, std::string subdir)
{
    ...
    easycpp::helpers::file_put_contents("log/" + (subdir == "" ? "" : subdir + "/") + "error." + easycpp::helpers::date("%Y%m%d") + ".log", data, FILE_APPEND);
}
...
void easycpp::helpers::log_debug(std::string tag, std::string msg, std::string subdir)
{
    ...
    easycpp::helpers::file_put_contents("log/" + (subdir == "" ? "" : subdir + "/") + "debug." + easycpp::helpers::date("%Y%m%d") + ".log", data, FILE_APPEND);
}
...
void easycpp::helpers::log_info(std::string tag, std::string msg, std::string subdir)
{
    ...
    easycpp::helpers::file_put_contents("log/" + (subdir == "" ? "" : subdir + "/") + "info." + easycpp::helpers::date("%Y%m%d") + ".log", data, FILE_APPEND);
}
