...
/// POST请求
int easycpp::helpers::http_post(const std::string& host, const std::string& port, const std::string& page, easycpp::libraries::JsonObject& form_data, std::string& reponse_data, int timeout)
{
...
/// POST请求(重载)
int easycpp::helpers::http_post(std::string url, easycpp::libraries::JsonObject& form_data, std::string& reponse_data, const int timeout)
{
    // 去掉url中的协议头
...
/// GET请求
int easycpp::helpers::http_get(const std::string& host, const std::string& port, const std::string& page, std::string& reponse_data, const int timeout)
{
    try {
        boost::asio::ip::tcp::iostream stream;
        stream.expires_from_now(boost::posix_time::seconds(timeout));
...
/// GET请求(重载)
int easycpp::helpers::http_get(std::string url, std::string& reponse_data, const int timeout)
{
    // 去掉url中的协议头
    if (url.find("http://") != std::string::npos) {
        url = url.substr(7);
...
/// url编码
std::string easycpp::helpers::urlencode(const std::string& str)
{
    std::string src = str;
...
/// url解码
std::string easycpp::helpers::urldecode(const std::string& str)
{
    std::string result;
    int hex = 0;
...
/// 生成http请求字符串
std::string easycpp::helpers::http_build_query(easycpp::libraries::JsonObject &query_data)
{
